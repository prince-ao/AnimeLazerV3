...
import { StyleSheet, Text, SafeAreaView , View, Dimensions, ScrollView, Image, TouchableOpacity} from "react-native";
...
const Search = ({navigation, navigate}) => {
  ...
  function getQueryRes(searchQuery) {
    ...
    .then(async function(res) {
      axios.get(`${API.url}/Animes/Search`, {
          headers: {
              'Content-Type': 'application/json',
        ...
        if (typeof res1.data.data !== 'undefined' && res1.data.data.length === 0) {
          (Platform.OS === 'android') 
          ? ToastAndroid.showWithGravity(`No results found for '${searchQuery}'`, 1500, ToastAndroid.BOTTOM)
          : Alert.alert(
          "Warning",
            ...
            searchResult.map((data, i) => {
              return (
                <View style={styles.searchContainer} key={i}>
                        ...
                        .then(async function(res) {
                          axios.get(`${API.url}/Animes/scrapeAnimeDetails`, {
                              ...
                              res1.data.data.map((data) => {
                                if (data === null) {
                                  (Platform.OS === 'android') 
                                  ? ToastAndroid.showWithGravity(res1.data.data, 1500, ToastAndroid.BOTTOM)
                                  : Alert.alert(
                                  "Warning",
                                  res1.data.data,
                                  [
                  ...
                  >
                    <Image source={{uri: data.uri}} style={styles.AnimeImage}/>
                    <View style={styles.info}>
                      <Text numberOfLines={2} style={styles.EnglishText}>{data.EnglishTitle}</Text>
                      <Text numberOfLines={2} style={styles.JapaneseText}>{data.JapaneseTitle}</Text>
                      <Text numberOfLines={1} style={styles.status}>{data.status}</Text>
                    </View>
                  </TouchableOpacity>
                </View>
              )
