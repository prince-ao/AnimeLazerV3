...
const EpisodeRoom = ({ navigation, route, truthy }) => {
	...
	const getStatus = async () => {
      ...
      */
			...
			if (ar.length > 1) {
				...
				const ar2 = [];
				for (let i = 0; i < ss_response.data.length; i++) {
					if (ss_response.data[i].node.title.includes(route.params.animeTitle)) {
	...
	const localHandleNewAnime = async () => {
		...
		db.transaction((tx) => {
			tx.executeSql(
				...
				[route.params.animeTitle, route.params.animeCover, 0, 0, "Plan To Watch"],
				(tx, res) => {},
				(err, errm) => console.log(errm)
			);
		});
	...
	const handleEpisode = async (data) => {
		axios
					...
					.then(async function (res1) {
						...
						if (
							...
							Platform.OS === "android"
								...
								: Alert.alert("Warning", "This video file cannot be played", [
										{
											text: "Cancel",
							...
							console.log(res1.data.data.m3u8);
							console.log(res1.data.data.referer);
							navigation.navigate("WatchRoom", {
								title: route.params.animeTitle + " Ep " + data.epNum,
        ...
        >
          <Ionicons name="heart-sharp" size={35} color="#5c94dd" />
					...
					<TouchableOpacity
						...
						<Ionicons name={"search"} size={30} color={truthy ? "white" : "black"} />
					</TouchableOpacity>
					<Modal
						animationType="fade"
						transparent={true}
            ...
            <Menu onSelect={value => alert(`You Clicked : ${value}`)}>
              <MenuTrigger > */}
					<TouchableOpacity
						...
						<Ionicons name={"filter"} size={30} color={truthy ? "white" : "black"} />
					</TouchableOpacity>
				</View>
				<View style={{ zIndex: 1 }}>
					<ScrollView
                      ...
                      />
									...
									<View style={styles(truthy).genDesc}>
										<Text style={styles(truthy).white}>
											Type:{" "}
											<Text style={styles(truthy).innerText}>{route.params.type} </Text>
										</Text>
										<Text
											numberOfLines={1}
											ellipsizeMode="tail"
										...
										<Text style={styles(truthy).white}>
											Status:{" "}
											<Text style={styles(truthy).innerText}>{route.params.status} </Text>
										</Text>
									</View>
								...
								{isAdded ? (
									<View
										...
										<View
											...
											<Picker
												item={status}
												items={data}
												onItemChange={logged == null ? localHandlePress : handlePress}
												title="Select"
															...
															<View
																...
																<TouchableOpacity
																	onPress={() => {
																		...
																		logged == null ? localHandleScore(0) : handleScore(0);
																	}}
																	activeOpacity={1}
																	style={{
																		marginLeft: 22,
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(1) : handleScore(1);
																	}}
																	activeOpacity={1}
																	style={{
																		//backgroundColor:
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(2) : handleScore(2);
																	}}
																	activeOpacity={1}
																	style={{
																		marginLeft: 22,
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(3) : handleScore(3);
																	}}
																	activeOpacity={1}
																	style={{
																		//backgroundColor:
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(4) : handleScore(4);
																	}}
																	activeOpacity={1}
																	style={{
																		marginLeft: 22,
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(5) : handleScore(5);
																	}}
																	activeOpacity={1}
																	style={{
																		//backgroundColor:
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(6) : handleScore(6);
																	}}
																	activeOpacity={1}
																	style={{
																		marginLeft: 22,
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(7) : handleScore(7);
																	}}
																	activeOpacity={1}
																	style={{
																		//backgroundColor:
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(8) : handleScore(8);
																	}}
																	activeOpacity={1}
																	style={{
																		marginLeft: 22,
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(9) : handleScore(9);
																	}}
																	activeOpacity={1}
																	style={{
																		//backgroundColor:
																...
																<TouchableOpacity
																	onPress={() => {
																		setModalVisible(false);
																		logged == null ? localHandleScore(10) : handleScore(10);
																	}}
																	activeOpacity={1}
																	style={{
																		marginLeft: 22,
									...
									<View style={{ marginBottom: 25 }}>
										<TouchableOpacity
											...
											<Modal animationType="slide" visible={loading} transparent={false}>
												<Image
