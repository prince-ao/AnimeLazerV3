...
const EpisodeRoom = ({ navigation, route, truthy }) => {
	...
	const [aToken, setAtoken] = useState(route.params.accessToken);
				...
				if (animeList.length > 0) {
					animeList.map((data, key) => {
						if (data.node.title == route.params.animeTitle) {
							...
							setEpisode(data.list_status.num_episodes_watched);
						}
					});
				}
			} else {
		...
		try {
			for (let i = 0; i < animeList.length; i++) {
				if (animeList[i].title == route.params.animeTitle) {
					...
					setRating(animeList[i].score);
					setAnimeKey(i);
					setAnimeId(animeList[i].timestamp);
					setEpisode(animeList[i].watchedEpisodes);
				}
			}
		} catch (err) {
			console.log(err);
	...
	const handleScore = async (score) => {
		if (aToken == null) {
			...
			fetch(
				`${API.url}/favorites/update/score?timestamp=${animeId}&score=${score}`,
				{
					method: "PUT",
					headers: {
						uid: uid,
				...
				.then((res) => {
					setIsLoading(false);
					animeList[animeKey].score = score;
					setRating(score);
				})
				.catch((err) => {
					setIsLoading(false);
		...
		try {
			...
			const response2 = await fetch(
				`https://api.myanimelist.net/v2/anime/${animeId}/my_list_status?score=${score}`,
				{
					method: "PUT",
					headers: {
	...
	const handleNewAnime = async () => {
		if (aToken == null) {
			try {
				...
				setIsLoading(true);
				await fetch(
					`${API.url}/favorites/add?timestamp=${timestamp}&anime=${
						route.params.animeTitle
					}&status=${"plan_to_watch"}&score=${0}&episodes_watched=${0}`,
					{
						method: "PUT",
						headers: {
							uid: uid,
		...
		try {
			setLoading(true);
			const response2 = await fetch(
				`https://api.myanimelist.net/v2/anime/${animeId}/my_list_status?score=${0}&status=${"plan_to_watch"}&num_watched_episodes=${0}`,
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/x-www-form-urlencoded",
	...
	const handleEpisodePress = async (ep) => {
		if (aToken == null) {
			...
			const uid = firebase.auth().currentUser.uid;
			fetch(
				`${API.url}/favorites/update/watched_episodes?timestamp=${animeId}&watched_episodes=${ep}`,
				{
					method: "PUT",
					headers: {
						uid: uid,
				...
				.then((res) => {
					animeList[animeKey].watchedEpisodes = ep;
					setEpisode(ep);
					setIsLoading(false);
				})
				.catch((err) => {
		...
		try {
			const response2 = await fetch(
				`https://api.myanimelist.net/v2/anime/${animeId}/my_list_status?num_watched_episodes=${ep}`,
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/x-www-form-urlencoded",
	...
	const handlePress = async (prop) => {
		...
		if (aToken == null) {
			...
			const uid = firebase.auth().currentUser.uid;
			fetch(
				`${API.url}/favorites/update/status?timestamp=${animeId}&status=${stat}`,
				{
					method: "PUT",
					headers: {
						uid: uid,
		...
		try {
			setIsLoading(true);
			const response2 = await fetch(
				`https://api.myanimelist.net/v2/anime/${animeId}/my_list_status?status=${stat}`,
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/x-www-form-urlencoded",
	...
	function handleEpisodeWatch(data) {
		setIsLoading(true);
		axios
			...
			.then(async function (res) {
				axios
					...
					.then(async function (res1) {
						await handleEpisodePress(data.epNum);
						setIsLoading(false);
						//console.log(res1)
						if (
